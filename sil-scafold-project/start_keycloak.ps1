# PowerShell script to start Keycloak and check its status
...
$keycloakExists = docker ps -a --filter "name=keycloak" --format "{{.Names}}" | Select-String "keycloak"
if (-not $keycloakExists) {
    Write-Host "Keycloak container does not exist. Creating and starting..." -ForegroundColor Yellow
    ...
    Write-Host "Waiting for Keycloak to start (this may take a minute)..." -ForegroundColor Yellow
    ...
    if (-not $keycloakRunning) {
        Write-Host "Keycloak container exists but is not running. Starting..." -ForegroundColor Yellow
        docker start keycloak
        Write-Host "Waiting for Keycloak to start..." -ForegroundColor Yellow
        Start-Sleep -Seconds 10
    } 
    else {
        Write-Host "✓ Keycloak container is already running" -ForegroundColor Green
    }
}
...
Write-Host "Checking if Keycloak is ready..." -ForegroundColor Cyan
Start-Sleep -Seconds 5

# Try to connect to Keycloak
try {
    $response = Invoke-WebRequest -Uri "http://localhost:8080/realms/master" -Method GET -ErrorAction Stop
    Write-Host "✓ Keycloak is accessible" -ForegroundColor Green
} catch {
    Write-Host "✗ Keycloak is not accessible: $_" -ForegroundColor Red
    Write-Host "It might still be starting up. Wait a minute and try the login again." -ForegroundColor Yellow
}

# Check if the SIL realm exists
try {
    $silRealmResponse = Invoke-WebRequest -Uri "http://localhost:8080/realms/sil" -Method GET -ErrorAction Stop
    Write-Host "✓ SIL realm exists" -ForegroundColor Green
} catch {
    Write-Host "SIL realm doesn't exist or is not accessible. Running the setup script..." -ForegroundColor Yellow
...
Write-Host "Issuer: http://localhost:8080/realms/sil" -ForegroundColor Yellow
